...
#include <stdio.h>
#include "hw_BME280.h"
#define ADDR BME280_DEFAULT_I2C_ADDR

void hw_BME280_init(i2c_inst_t* i2c_bus) {
    uint8_t data;

    if (i2c_readfrom_mem(i2c_bus, ADDR, BME280_REG_CHIPID, &data, 1))
    {
    ...
    // Reset the device.
    i2c_write8(i2c_bus, ADDR, BME280_REG_RESET, BME280_BME_RESET_FLAG );
    hw_BME280_set_data_acq_options(i2c_bus);
}
...
void hw_BME280_set_data_acq_options(i2c_inst_t* i2c_bus) {
    ...
    i2c_write8(i2c_bus, ADDR, BME280_REG_CTRL_HUM, reg_ctrl_hum_val);
    ...
    i2c_write8(i2c_bus, ADDR, BME280_REG_CTRL_MEAS, reg_ctrl_meas_val);
    ...
    i2c_write8(i2c_bus, ADDR, BME280_REG_CONFIG, reg_config_val);
}

bool hw_BME280_is_busy(i2c_inst_t* i2c_bus) {
    // returns true if the device is busy making a measurement.
    uint8_t status;
    i2c_readfrom_mem(i2c_bus, ADDR, BME280_REG_STATUS, &status, 1);
    bool busy = (status & 0x08) >> 3;
    return busy;
}
...
void hw_BME280_get_calib_params(i2c_inst_t* i2c_bus,  hw_BME280_calib_param* params) {
    ...
    i2c_readfrom_mem(i2c_bus, ADDR, BME280_REG_DIG_T1_LSB, buf, BME280_NUM_TP_CALIB_BYTES);
    ...
    i2c_readfrom_mem(i2c_bus, ADDR, BME280_REG_DIG_H1, buf1, 1);
    params->dig_h1 = buf[0];

    i2c_readfrom_mem(i2c_bus, ADDR, BME280_REG_DIG_H2, buf1, BME280_NUM_H_CALIB_BYTES);
    params->dig_h2 = (int16_t)(buf1[1] << 8) | buf1[0];
    params->dig_h3 = buf1[2];
    params->dig_h4 = (int16_t)(buf1[3] << 4) | (buf1[4] & 0xF);
    params->dig_h5 = (int16_t)(buf1[5] << 4) | (buf1[4] >> 4);
    params->dig_h6 = (int8_t)buf1[6];

}

void hw_BME280_read_raw(i2c_inst_t* i2c_bus, int32_t* adc_T, int32_t* adc_P, int32_t* adc_H) {
    ...
    hw_BME280_set_data_acq_options(i2c_bus);
    int spins = 0;
    while (hw_BME280_is_busy(i2c_bus)) {
        spins++;
        sleep_ms(10);
    }
    //printf(" Spins = %d\n", spins);
    ...
    uint8_t buf[8];
    i2c_readfrom_mem(i2c_bus, ADDR, BME280_REG_PRESSURE_MSB, buf, 8);
...
int32_t hw_BME280_convert(int32_t adc_T, hw_BME280_calib_param* params) {
...
int32_t hw_BME280_convert_temp(int32_t adc_T, hw_BME280_calib_param* params) {
    ...
    int32_t t_fine = hw_BME280_convert(adc_T, params);
...
int32_t hw_BME280_convert_humidity(int32_t adc_H, int32_t adc_T, hw_BME280_calib_param* params) {
    ...
    var1 = (((adc_H<<14) - (params->dig_h4<<20) - (params->dig_h5 * h)) + 16384) >>15;
    var2 = (((((((h*params->dig_h6)>>10) * (((h*params->dig_h3)>>11) + 32768))>>10)
                + 2097152)*params->dig_h2 + 8192)>>14);

    h = var1 * var2;
    h = h - (((((h>>15)*(h>>15))>>7)*params->dig_h1)>>4);
    h = (h < 0 ? 0 : h);
...
void hw_BME280_read_values(i2c_inst_t* i2c_bus, hw_BME280_calib_param* params, 
        ...
        hw_BME280_read_raw(i2c_bus, &adc_T, &adc_P, &adc_H);
        int32_t temp_i32 = hw_BME280_convert_temp(adc_T, params);
        int32_t press_i32 = hw_BME280_convert_pressure(adc_P, adc_T, params);
        int32_t humid_i32 = hw_BME280_convert_humidity(adc_H, adc_T, params);
